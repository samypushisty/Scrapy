...
class ExampleSpider(scrapy.Spider):
    ...
    def parse_page(self, response, href):
        yield {"title": response.xpath("//h1[@class='catalog-masthead__title js-nav-header']/text()").get().strip(),
               "href": href,
               "price": response.xpath("//a[@class='offers-description__link "
                                       "offers-description__link_nodecor "
                                       "js-description-price-link']/text()").get().strip()[:-3]+"р",
